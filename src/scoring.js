...
if(f.eprescribe&&v.eprescribe)s+=6;if(f.injectableTracking&&v.injectableTracking)s+=7;if(f.recallTraceability&&v.recallTraceability)s+=10;
...
return s}
export function fitLabel(score,margin=0,gate={eligible:true},coverage={verified:0,total:0}){if(!gate.eligible)return "Does not meet a required need";const evidenceRatio=coverage.total?coverage.verified/coverage.total:1;if(score>=82&&margin>=5&&evidenceRatio>=.75)return "Strong fit";if(score>=70&&evidenceRatio>=.5)return "Good fit";return "Possible fit"}
...
export const evidence={
 ...
 Zenoti:{
  ...
  injectableTracking:{status:"verified",source:"https://www.zenoti.com/medical-spa-software/injectable-tracking",checkedAt:"2026-09-22",note:"Vendor documents vial/lot registration, expiration dates and dispensing records."},
  recallTraceability:{status:"verified",source:"https://help.zenoti.com/en/inventory/reports/inventory-traceability-report-for-batch-tracked-products.html",checkedAt:"2026-09-22",note:"Vendor documents filtering by recalled batch to identify affected locations, patients, administering employees and remaining stock."},
...
Object.assign(evidence,{
 Mangomint:{
  ...
  injectableTracking:{status:"verified",source:"https://www.mangomint.com/blog/meet-image-markup-a-powerful-new-addition-to-integrated-forms/",checkedAt:"2026-09-22",note:"Vendor documents injectable charting with product details, units, batch numbers and expiration dates in HIPAA-compliant client records. Inventory usage is tracked separately; recall workflow is not explicitly documented."}
 },
 Phorest:{
  ...
  injectableTracking:{status:"verified",source:"https://www.phorest.com/us/blog/compliant-med-spa-clinic-software/",checkedAt:"2026-09-22",note:"Vendor documents administered units, lot numbers and injection sites in searchable, auditable treatment notes. Recall workflow and expiration tracking are not explicitly documented and should be verified in demo."}
 },
 Vagaro:{
